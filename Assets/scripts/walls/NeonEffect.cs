...
public class NeonEffect : MonoBehaviour
{
	...
	void Start ()
	{
	    ...
	    List<Vector3> vertex = new List<Vector3>(meshFilter.mesh.vertices);
        int vertexSize = vertex.Count;
        vertex.AddRange(new Vector3[meshFilter.mesh.vertices.Length]);
        bool[] boolVertex = new bool[vertex.Count];

	    List<int> triangles = new List<int>(meshFilter.mesh.triangles);
        List<Vector3> normals = new List<Vector3>(meshFilter.mesh.normals);
        normals.AddRange(new Vector3[meshFilter.mesh.vertices.Length]);
